import { SearchSubTenantsResponse, IGetTenantsUserCountResponse, FronteggPaginationWrapper } from '@frontegg/rest-api';
/**
 * @param param0 accounts without users count
 * @param tenantsUsersCountArray users count for each account array
 * @returns array of accounts with users count
 */
export declare const getAccountsWithUsersCount: ({ items: accounts }: Pick<FronteggPaginationWrapper<SearchSubTenantsResponse>, "items">, tenantsUsersCountArray: IGetTenantsUserCountResponse[]) => {
    name: string;
    tenantId: string;
    numberOfUsers: number;
    createdAt: Date;
    metadata: import("@frontegg/rest-api").SearchSubTenantMetadata;
    isReseller: boolean | undefined;
    subAccountAccessType: import("@frontegg/rest-api").SubAccountAccessTypeEnum | undefined;
}[];
